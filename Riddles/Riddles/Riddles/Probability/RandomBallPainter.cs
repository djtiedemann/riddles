...
namespace Riddles.Probability
{
	...
	public class RandomBallPainter
	{
		...
		public double? GetExpectedNumberOfTurnsToPaintBalls(int numBalls)
		{
			...
			var statesToProcess = new List<State>();
			var stateTransitionDictionary = new Dictionary<State, Dictionary<State, double>>();
			
			var initialStateBallDistribution = new Dictionary<int, int> { { 1, numBalls } };
			var initialState = new State(initialStateBallDistribution);
			...
			var termDictionary = new Dictionary<State, int>();			
			var nextStateId = 0;
			...
			foreach(var state in stateTransitionDictionary.Keys)
			{
				...
				if(this.IsStateTerminalState(state, numBalls))
				{
		...
		public Dictionary<State, double> GetStateTransitions(State currentState, int numBalls)
		{
			...
			{
				var probabilityFirstTypeOfBallDrawn = ((double)key1 * (double)currentState.BallDistributionForState[key1]) / (double)numBalls;
				foreach (var key2 in currentState.BallDistributionForState.Keys)
				{
					if (key1 == key2)
					{
						...
						var probabilityOfDrawingSameBallTwice = (key1 - 1) / ((double)numBalls - 1);
						var currentStateClone = new State(this.DeepCloneBallDistribution(currentState));
						...
						if (currentState.BallDistributionForState[key2] == 1)
						{
							continue;
						}
						...
						var probabilityOfDrawingDifferentBall 
							= (double)key2 * ((double)currentState.BallDistributionForState[key2] - 1) / ((double)numBalls - 1);
					...
					else
					{
						var probabilityDrawingSecond 
							= ((double)key2 * (double)currentState.BallDistributionForState[key2]) / ((double)numBalls - 1);
		...
		public Dictionary<int, int> DeepCloneBallDistribution(State state)
		{
			...
			{
				clone[key] = state.BallDistributionForState[key];
			}
			return clone;
		}
